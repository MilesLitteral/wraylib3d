...
module LambdaCube.DirectX.Backend where
...
compileProgram p = do
    ...
    let createAndAttach src t = do
            o <- glCreateShader t -- ID3D11VertexShader
            compileShader o [src] -- d3dCompile source sourceName defines pInclude entryPoint target compileFlags effectFlags 
            glAttachShader po o --
...
compileRenderTarget :: Vector TextureDescriptor -> Vector DX11Texture -> RenderTarget -> IO DX11RenderTarget
