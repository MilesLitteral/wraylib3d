{-# LANGUAGE TupleSections, MonadComprehensions, RecordWildCards, LambdaCase, TypeApplications,  FlexibleContexts, DataKinds #-}
{-# LANGUAGE CPP                   #-}
{-# LANGUAGE MultiParamTypeClasses #-}
{-# LANGUAGE ScopedTypeVariables   #-}
{-# LANGUAGE StrictData            #-}

module LambdaCube.Vulkan.Backend where
...
import Graphics.UI.GLFW (getRequiredInstanceExtensions)
...
import LambdaCube.Vulkan.Data
import LambdaCube.Vulkan.Type
import LambdaCube.Vulkan.Input
...
-- type Program' a = LambdaCube.Vulkan.Backend.Program (Either VulkanException a) a


-- runProgram :: (Either VulkanException a -> IO r)
--            -> LambdaCube.Vulkan.Backend.Program r a -> IO r
...
--               -> LambdaCube.Vulkan.Backend.Program r a -- ^ allocate resource
--               -> LambdaCube.Vulkan.Backend.Program r a
-- allocResource free alloc = LambdaCube.Vulkan.Backend.Program $ \ref c ->
...
-- allocResource' :: (a -> LambdaCube.Vulkan.Backend.Program' ()) -- ^ free resource
--                -> LambdaCube.Vulkan.Backend.Program r a -- ^ allocate resource
--                -> LambdaCube.Vulkan.Backend.Program r (a, LambdaCube.Vulkan.Backend.Program r ())
-- allocResource' free alloc = LambdaCube.Vulkan.Backend.Program $ \ref c ->
--   unProgram alloc ref $ \case
--     Left e -> c (Left e)
--     Right a -> c (Right (a, LambdaCube.Vulkan.Backend.Program $ \ref' c' ->
--                                  c' (Right ()) >>=
...
-- locally :: LambdaCube.Vulkan.Backend.Program' a
--         -> LambdaCube.Vulkan.Backend.Program r a
-- locally p = LambdaCube.Vulkan.Backend.Program $ \ref c -> unProgram p ref pure >>= c
-- {-# INLINE locally #-}

-- instance Functor (LambdaCube.Vulkan.Backend.Program r) where
--   fmap f p = LambdaCube.Vulkan.Backend.Program $ \ref c -> unProgram p ref (c . fmap f)
--   {-# INLINE fmap #-}

-- instance Applicative (LambdaCube.Vulkan.Backend.Program r) where
--   pure x = LambdaCube.Vulkan.Backend.Program $ const ($ Right x)
--   {-# INLINE pure #-}
--   pf <*> px = LambdaCube.Vulkan.Backend.Program $
--     \ref c -> unProgram pf ref $ \g -> unProgram px ref (c . (g <*>) )
--   {-# INLINE (<*>) #-}

-- instance Monad (LambdaCube.Vulkan.Backend.Program r) where
--   return = pure
--   {-# INLINE return #-}
--   px >>= k = LambdaCube.Vulkan.Backend.Program $
--     \ref c -> unProgram px ref $ \case
--       Right x -> unProgram (k x) ref c
--       Left e  -> c (Left e)
--   {-# INLINE (>>=) #-}

-- instance MonadIO (LambdaCube.Vulkan.Backend.Program r) where
--   liftIO m = LambdaCube.Vulkan.Backend.Program $ const (Right <$> m >>=)
--   {-# INLINE liftIO #-}

-- instance MonadState ProgramState (LambdaCube.Vulkan.Backend.Program r) where
--   get     = LambdaCube.Vulkan.Backend.Program $ \ref -> (Right <$> readIORef ref >>=)
--   {-# INLINE get #-}
--   put s   = LambdaCube.Vulkan.Backend.Program $ \ref -> (Right <$> writeIORef ref s >>=)
--   {-# INLINE put #-}
--   state f = LambdaCube.Vulkan.Backend.Program $ \ref -> (Right <$> atomicModifyIORef' ref (swap . f) >>=)
--   {-# INLINE state #-}

-- instance MonadError VulkanException (LambdaCube.Vulkan.Backend.Program r) where
--   throwError e = LambdaCube.Vulkan.Backend.Program $ const ($ Left e)
--   {-# INLINE throwError #-}
--   catchError px catcher = LambdaCube.Vulkan.Backend.Program $ \ref c -> unProgram px ref $ \case
--     Left e  -> unProgram (catcher e) ref c
--     Right r -> c (Right r)
--   {-# INLINE catchError #-}

-- -- | Throw VulkanException without error code
-- throwVkMsg :: HasCallStack => String -> LambdaCube.Vulkan.Backend.Program r a
...
-- bracket :: LambdaCube.Vulkan.Backend.Program r a        -- ^ computation to run first (\"acquire resource\")
--         -> (a -> LambdaCube.Vulkan.Backend.Program r b) -- ^ computation to run last (\"release resource\")
--         -> (a -> LambdaCube.Vulkan.Backend.Program r c) -- ^ computation to run in-between
--         -> LambdaCube.Vulkan.Backend.Program r c        -- returns the value from the in-between computation
-- bracket before after thing = do
--   a <- before
--   er <- try (thing a)
--   _ <- after a
--   LambdaCube.Vulkan.Backend.Program $ const ($ er)
-- {-# INLINE bracket #-}
...
-- --   This does not handle vanilla Haskell exceptions, and should only be used
-- --   to catch the exception defined in this module.
-- finally :: LambdaCube.Vulkan.Backend.Program r a  -- ^ computation to run first
--         -> LambdaCube.Vulkan.Backend.Program r b  -- ^ computation to run afterward (even if an exception
--                         -- was raised)
--         -> LambdaCube.Vulkan.Backend.Program r a  -- returns the value from the first computation
-- finally a sequel = do
--   er <- try a
--   _ <- sequel
--   LambdaCube.Vulkan.Backend.Program $ const ($ er)
...
-- --   This does not handle vanilla Haskell exceptions, and should only be used
-- --   to catch the exception defined in this module.
-- try :: LambdaCube.Vulkan.Backend.Program r a -> LambdaCube.Vulkan.Backend.Program r (Either VulkanException a)
-- try a = LambdaCube.Vulkan.Backend.Program $ \ref c -> unProgram a ref $ c . Right
-- {-# INLINE try #-}


-- -- | Run vulkan command, throwing an exception if its result is an error.
-- runVk :: HasCallStack => IO VkResult -> LambdaCube.Vulkan.Backend.Program r ()
-- runVk action = do
--   r <- liftIO action
...
-- instance Logger.MonadLogger (LambdaCube.Vulkan.Backend.Program r) where
...
-- logDebug :: HasCallStack => String -> LambdaCube.Vulkan.Backend.Program r ()
-- #ifdef DEVELOPMENT
...
-- logInfo :: HasCallStack => String -> LambdaCube.Vulkan.Backend.Program r ()
-- logInfo = LoggerCS.logInfo . fromString
-- {-# INLINE logInfo #-}

-- logWarn :: HasCallStack => String -> LambdaCube.Vulkan.Backend.Program r ()
-- logWarn = LoggerCS.logWarn . fromString
-- {-# INLINE logWarn #-}

-- logError :: HasCallStack => String -> LambdaCube.Vulkan.Backend.Program r ()
...
--               )
--            -> (a -> Program' b) -> LambdaCube.Vulkan.Backend.Program r b
-- liftIOWith iof pf = LambdaCube.Vulkan.Backend.Program $ \ref c ->
...
--           => a
--           -> (Ptr a -> LambdaCube.Vulkan.Backend.Program' b)
--           -> LambdaCube.Vulkan.Backend.Program r b
-- withVkPtr x = liftIOWith (withPtr x)
-- {-# INLINE withVkPtr #-}

-- -- | Low latency time in seconds since the start
-- getTime :: LambdaCube.Vulkan.Backend.Program r Double
-- getTime = do
...
-- occupyThreadAndFork :: LambdaCube.Vulkan.Backend.Program r () -- ^ the program to run in the main thread
--                     -> LambdaCube.Vulkan.Backend.Program' () -- ^ the program to run in a separate thread
--                     -> LambdaCube.Vulkan.Backend.Program r ()
-- occupyThreadAndFork mainProg deputyProg = LambdaCube.Vulkan.Backend.Program $ \ref c -> do
...
-- loop :: LambdaCube.Vulkan.Backend.Program' LoopControl -> LambdaCube.Vulkan.Backend.Program r ()
-- loop action = do
...
withVulkanInstanceExt :: String -> [CString] -> (VkInstance -> IO ()) -> IO ()
withVulkanInstanceExt title exts action = do
...
vkPickPhysicalDevice :: VkInstance -> IO VkPhysicalDevice
vkPickPhysicalDevice vkInstance = do
        ...
        where
          ...
          selectFirstSuitable (x:xs) = isDeviceSuitable x >>= \yes -> if yes then pure x else selectFirstSuitable xs

vkIsDeviceSuitable :: VkPhysicalDevice -> IO Bool
vkIsDeviceSuitable _ = pure True
 
-- This all Setups Scenes in terms of OpenGL
...
compileProgram :: LambdaCube.IR.Program -> IO GLProgram
compileProgram p = do
    po <- glCreateProgram
