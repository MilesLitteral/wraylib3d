...
--     "IDP3"  <- getByteString 4
--     version <- getInt
--     when (version /= 15) $ fail "unsupported GLB version"
--     name  <- getLowerCaseString 64
--     flags <- getInt
--     [nFrames,nTags,nSurfaces,nSkins] <- replicateM 4 getInt
--     [oFrames,oTags,oSurfaces,oEnd] <- replicateM 4 getInt
--     return $ GLBModel
--       { glbFrames    = getV oFrames nFrames getFrame dat
--       , glbTags      = (\v -> HashMap.fromList [(tgName t,t) | t <- V.toList v]) <$> getV oTags nFrames (V.replicateM nTags getTag) dat
--       , glbSurfaces  = getV oSurfaces nSurfaces getSurface dat
--       }
