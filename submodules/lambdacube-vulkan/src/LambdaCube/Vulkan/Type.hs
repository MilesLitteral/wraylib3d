...
module LambdaCube.Vulkan.Type where
...
-}
data VkUniform = forall a. Storable a => VkUniform !InputType !(IORef a)

instance Show VkUniform where
    show (VkUniform t _) = "VkUniform " ++ show t
...
data GLStorage
    ...
    , uniformSetup  :: Map String VkUniform
...
data Object -- internal type
    ...
    , objUniSetup   :: Map String VkUniform  
    , objOrder      :: IORef Int
...
data GLObjectCommand
    = GLSetUniform              !GLint !VkUniform
    | GLBindTexture             !GLenum !(IORef GLint) !VkUniform               -- binds the texture from the vkuniform to the specified texture unit and target
