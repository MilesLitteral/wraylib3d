...
lAttrQualifier :: P ()
lAttrQualifier = lexeme $ string "[[" >> return ()

rAttrQualifier :: P ()
rAttrQualifier = lexeme $ string "]]" >> return ()
...
typeSpecifierNonArray = choice
  ...
  , keyword "double"        >> return Double
  , keyword "char"          >> return Char
  , keyword "uchar"         >> return UChar
  , keyword "bool"          >> return Bool
  , keyword "short"         >> return Short
  , keyword "ushort"        >> return UShort
  , keyword "long"          >> return Long
  , keyword "ulong"         >> return ULong
  , keyword "half"          >> return Half
  , keyword "half4"         >> return Half4
  , keyword "uint"          >> return UInt
  , keyword "uint2"         >> return UInt2
  , keyword "size_t"        >> return Size_t
  , keyword "ptrdiff_t"     >> return Ptrdiff_t
  , keyword "intptr_t"      >> return Intptr_t
  , keyword "uintpyt_t"     >> return Uintpyt_t
  , keyword "kernel"        >> return Kernel
  , keyword "device"        >> return Device
  , keyword "texture"       >> return Texture
  , keyword "texture2d"     >> return Texture2d
  , keyword "access::read"  >> return AccessRead
  , keyword "access::write" >> return AccessWrite
  --, attributeQualifier --structSpecifier  -- , keyword "[[thread_position_in_grid]]" >> return ThreadPositionInGrid --[[uint2]] --[[texture()]]
...
attributeQualifier :: P TypeSpecifierNonArray
attributeQualifier = do
  lAttrQualifier
  i <- identifier
  d <- qualifierDeclarationList
  rAttrQualifier 
  return $ AttributeQualifier (Just i) d--(head d) --StructSpecifier i d

qualifierDeclarationList :: P [Field]
qualifierDeclarationList = many1 qualifierDeclaration

qualifierDeclaration :: P Field
qualifierDeclaration = do
  q <- optionMaybe typeQualifier
  s <- typeSpecifier
  lparen
  l <- qualifierDeclarator
  l <- structDeclarator
  rparen
  return $ Field q s [l] 

qualifierDeclaratorList :: P [QualifierDeclarator]
qualifierDeclaratorList = qualifierDeclarator `sepBy` comma

qualifierDeclarator :: P QualifierDeclarator
qualifierDeclarator = do
  i <- identifier
  choice
    [ do lbracket
         e <- optionMaybe constantExpression
         rbracket
         return $ QualifierDeclarator i (Just e)
    , return $ QualifierDeclarator i Nothing
    ]
