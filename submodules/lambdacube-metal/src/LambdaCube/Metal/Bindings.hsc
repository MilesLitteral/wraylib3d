...
metalFalse, metalTrue, metalNull :: FRValue
metalFalse = intPtrToPtr 0x00
metalTrue  = intPtrToPtr 0x14
metalNull  = intPtrToPtr 0x08

mtype :: FRValue -> IO RType
mtype v = rubyType v >>= \x -> case x of
...
metal_string_value_cstr :: FRValue -> IO String
metal_string_value_cstr v = do
    pv <- new v
...
metal_define_global_function :: String -- ^ Name of the function
                          -> FunPtr a -- ^ Pointer to the function (created with something like `mkRegistered0`)
                          -> Int -- ^ Number of arguments the function accepts.
                          -> IO ()
metal_define_global_function s f i = withCString s (\cs -> c_rb_define_global_function cs f i)
...
metal_define_method
  ...
  -> IO ()
metal_define_method r s f i = withCString s (\cs -> c_rb_define_method r cs f i)

-- | Defines a singleton method.
--
-- Arguments are the same as 'rb_define_method'.
metal_define_singleton_method :: FRValue -> String -> FunPtr a -> Int -> IO ()
metal_define_singleton_method r s f i = withCString s (\cs -> c_rb_define_singleton_method r cs f i)

-- | Defines a module function (@module_function@) to a module.
--
-- Arguments are the same as 'rb_define_method'.
metal_define_module_function :: FRValue -> String -> FunPtr a -> Int -> IO ()
metal_define_module_function r s f i = withCString s (\cs -> c_rb_define_module_function r cs f i)

metal_define_class :: String  -> FRValue -> IO FRValue
metal_define_class str rv = withCString str (\s -> c_rb_define_class s rv)

metal_str_new2 :: String -> IO FRValue
metal_str_new2 str = withCString str c_rb_str_new2

metal_define_module :: String -> IO ()
metal_define_module str = withCString str c_rb_define_module

-- | Sets an instance variable
metal_iv_set :: FRValue -- ^ The object
          -> String -- ^ The variable name (without the @)
          -> FRValue -- ^ The value to set
          -> IO FRValue -- ^ The value you just set
metal_iv_set obj varname vaFRValue = withCString ('@' : varname) $ \cvarname -> c_rb_iv_set obj cvarname vaFRValue

-- | Loads a ruby script (and executes it).
metal_load_protect :: String -- ^ Path to the script
...
metal_funcall :: FRValue -> RID -> [FRValue] -> IO FRValue
metal_funcall a b []          = c_metal_funcall_0 a b 0
metal_funcall a b [d]         = c_metal_funcall_1 a b 1 d
metal_funcall a b [d,e]       = c_metal_funcall_2 a b 2 d e
metal_funcall a b [d,e,f]     = c_metal_funcall_3 a b 3 d e f
metal_funcall a b [d,e,f,g]   = c_metal_funcall_4 a b 4 d e f g
metal_funcall a b [d,e,f,g,h] = c_metal_funcall_5 a b 5 d e f g h
metal_funcall _ _ _           = error "Can't call functions with that many arguments"

metalMethodCall :: String -> String -> [FRValue] -> IO FRValue
metalMethodCall classname methodname args = do
    c <- getClass classname
    m <- metal_intern methodname
    metal_funcall c m args
...
getObjClass :: String -> IO FRValue
getObjClass s = do
    i <- rb_intern s
    o <- peek metal_ObjCObject
    rb_const_get o i

metal_gv_get :: String -> IO FRValue
metal_gv_get s = withCString s c_rb_gv_get

metal_intern :: String -> IO RID
metal_intern s = withCString s c_rb_intern

metal_string_value_ptr :: FRValue -> IO String
metal_string_value_ptr rv = do
    rvp <- new rv
