...
class AAPLRenderer
{
    ...
    MTL::Buffer _objectParameters;

    // The Metal buffers storing per frame uniform data
    MTL::Buffer _frameStateBuffer[AAPLMaxFramesInFlight];
...
AAPLRenderer initWithMetalKitView(MTKView * mtkView)
{
    ...
    if(self)
    {
        ...
        mtkView.clearColor = MTL::ClearColorMake(0.0, 0.0, 0.5, 1.0f);
        ...
        mtkView.depthStencilPixelFormat = MTL::PixelFormatDepth32Float;
        mtkView.colorPixelFormat = MTL::PixelFormatBGRA8Unorm_sRGB;
        ...
        _renderPipelineState = _device.newRenderPipelineStateWithDescriptor(pipelineStateDescriptor, &error);

        NS::Assert(_renderPipelineState, "Failed to create pipeline state: %@", error);

        MTL::Function GPUCommandEncodingKernel = defaultLibrary.newFunctionWithName("cullMeshesAndEncodeCommands");
        _computePipelineState = _device.newComputePipelineStateWithFunction(GPUCommandEncodingKernel, &error);

        NS::Assert(_computePipelineState ,"Failed to create compute pipeline state: %@", error);
        ...
        AAPLObjectMesh *tempMeshes;
        {
            ...
            for(int objectIdx = 0; objectIdx < AAPLNumObjects; objectIdx++)
            {
                ...
                tempMeshes[objectIdx] = self.newGearMeshWithNumTeeth(numTeeth, innerRatio, toothWidth, toothSlope);
            }
        }

        // Create and fill array containing parameters for each object
        {
            NS::UInteger objectParameterArraySize = AAPLNumObjects * sizeof(AAPLObjectPerameters);
            _objectParameters = _device.newBufferWithLength(objectParameterArraySize, 0);
        ...
        // Create a single buffer with vertices for all gears
        {
            ...
            _vertexBuffer = _device.newBufferWithLength(bufferSize, 0);
        ...
        {
            _frameStateBuffer[i] = _device.newBufferWithLength(sizeof(AAPLFrameState), MTL::ResourceStorageModeShared);
            _frameStateBuffer[i].label = NS::String.stringWithFormat:("Frame state buffer %d", i);
        }

        MTL::IndirectCommandBufferDescriptor* icbDescriptor = new MTL::IndirectCommandBufferDescriptor;
        
        // Only standard (non-indexed) draw commands are allowed.
        icbDescriptor.commandTypes = MTL::IndirectCommandTypeDraw;
        ...
        // memory
        _indirectCommandBuffer = _device.newIndirectCommandBufferWithDescriptor(icbDescriptor, AAPLNumObjects, MTL::ResourceStorageModePrivate);
        _indirectCommandBuffer.label = "Scene ICB";


        MTL::ArgumentEncoder argumentEncoder = GPUCommandEncodingKernel.newArgumentEncoderWithBufferIndex(AAPLKernelBufferIndexCommandBufferContainer);

        _icbArgumentBuffer = [_device, argumentEncoder.encodedLength, MTL::ResourceStorageModeShared];
...
{
    NS::Assert(numTeeth >= 3, "Can only build a gear with at least 3 teeth");
    NS::Assert(toothWidth + 2 * toothSlope < 1.0, "Configuration of gear invalid");
...
void drawInMTKView(MTKView * view)
{
    ...
    resetBlitEncoder.resetCommandsInBuffer(_indirectCommandBuffer, NS::MakeRange(0, AAPLNumObjects));
    ...
    computeEncoder.setBuffer(_frameStateBuffer[_inFlightIndex], 0, AAPLKernelBufferIndexFrameState);
    computeEncoder.setBuffer(_objectParameters,  0, AAPLKernelBufferIndexObjectParams);
    computeEncoder.setBuffer(_vertexBuffer,      0, AAPLKernelBufferIndexVertices);
    ...
    NS::UInteger threadExecutionWidth = _computePipelineState.threadExecutionWidth;
    computeEncoder.useResource(_indirectCommandBuffer, MTL::ResourceUsageWrite);
    computeEncoder.dispatchThreads(MTL::SizeMake(AAPLNumObjects, 1, 1), MTL::SizeMake(threadExecutionWidth, 1, 1));
    ...
    optimizeBlitEncoder.optimizeIndirectCommandBuffer(_indirectCommandBuffer, NS::MakeRange(0, AAPLNumObjects));
    ...
    if(renderPassDescriptor != nil)
    {
        ...
        renderEncoder.useResource(_vertexBuffer, MTL::ResourceUsageRead);

        renderEncoder.useResource(_objectParameters, MTL::ResourceUsageRead);
        renderEncoder.useResource(_frameStateBuffer[_inFlightIndex], MTL::ResourceUsageRead);

        // Draw everything in the indirect command buffer
        renderEncoder.executeCommandsInBuffer(_indirectCommandBuffer.withRange(NS::MakeRange(0, AAPLNumObjects)));
        ...
        commandBuffer.presentDrawable(view.currentDrawable, 0.02);
#else
