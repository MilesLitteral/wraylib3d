...
import LambdaCube.XR
import LambdaCube.XR.Mesh
import TestData
...
main = do
  let instanceInfo = XrInstanceCreateInfo {
    iciType                  :: XrStructureType,
    iciCreateFlags           :: XrInstanceCreateFlags,
    iciApplicationInfo       :: XrApplicationInfo,
    iciEnabledApiLayerCount  :: Int, 
    iciEnabledApiLayerNames  :: String,
    ...
    xrBinding       = XrGraphicsBindingOpenGLWin32KHR XR_TYPE_GRAPHICS_BINDING_OPENGL_WIN32_KHR renderer win
    xrRequirement   = XrGraphicsRequirementsOpenGLKHR XR_TYPE_GRAPHICS_REQUIREMENTS_OPENGL_KHR  "1.0.0"  "5.2.5"
    xrInstance      = XrInstance handle instanceInfo 
    xrSession       = xrCreateSession xrInstance instanceInfo XR_SESSION_STATE_READY 

  xrCreateInstance instanceInfo xrInstance
  win <- initWindow "LambdaCube 3D OpenXR 1.0 Backend" 256 256
  xrGetSystem xrInstance instanceInfo (XrSystemId 1)
  ...
  forever $ catchAll (setupConnection win) $ \_ -> do
    GLFW.pollEvents
    xrPollEvent xrInstance --eventDataBuffer
    threadDelay 100000
