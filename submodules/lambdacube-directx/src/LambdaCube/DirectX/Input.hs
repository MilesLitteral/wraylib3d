...
import LambdaCube.DirectX.Type as T
...
allocStorage sch = do
    ...
    seed <- newIORef 0
    slotV <- V.replicateM len $ newIORef (DX11Slot IM.empty V.empty Ordered)
    size <- newIORef (0,0)
...
removeObject :: DX11Storage -> Object -> IO ()
removeObject p obj = modifyIORef (slotVector p ! objSlot obj) $ \(DX11Slot objs _ _) -> DX11Slot (IM.delete (objId obj) objs) V.empty Generate

enableObject :: Object -> Bool -> IO ()
enableObject obj b = writeIORef (objEnabled obj) b

setObjectOrder :: DX11Storage -> Object -> Int -> IO ()
setObjectOrder p obj i = do
    writeIORef (objOrder obj) i
    modifyIORef (slotVector p ! objSlot obj) $ \(DX11Slot objs sorted _) -> DX11Slot objs sorted Reorder

objectUniformSetter :: Object -> Map GLUniformName InputSetter
objectUniformSetter = objUniSetter

setScreenSize :: DX11Storage -> Word -> Word -> IO ()
setScreenSize p w h = writeIORef (screenSize p) (w,h)

sortSlotObjects :: DX11Storage -> IO ()
